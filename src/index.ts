...
const generateThumbnailFromImage = (image: string, outPath: string) => {
  return new Promise<ImgData>(async (resolve, reject) => {
    ...
    if (!colors || !mainColor) {
      reject("Could not get colors from image");
      return;
    }
    resolve({
      fullSize: image,
...
export const getAll = async (images: string[], outDir:string,inDir:string) => {
  const items: ImgData[] = [];
